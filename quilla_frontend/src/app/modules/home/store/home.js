  ...
  actions: {
    ...
    async obtenerUbicacion(reload = true) {
      try {
        const pos = await Geolocation.getCurrentPosition({
          ...
          timeout: 5000, // Evita esperas infinitas si el GPS demora en responder
          maximumAge: 0
        });
    ...
    async startWatchingPosition() {
      try {
        ...
        // Carga inmediata del mapa (con Cusco por defecto si no hay coordenadas de GPS aún)
        await this.getMapa();

        // Obtener ubicación en segundo plano sin bloquear el hilo principal de renderizado
        this.obtenerUbicacion(true).then(() => {
          console.log('Ubicación inicial obtenida exitosamente');
        }).catch(err => {
          console.error('Error al resolver la ubicación inicial:', err);
        });
    ...
    async getMapa() {
      ...
      // Evita poner isMapLoaded en false si el mapa ya se cargó previamente
      // para prevenir parpadeos y loaders innecesarios en la UI
      if (!this.map) {
